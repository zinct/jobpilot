        ...
        <div className="space-y-6">
          ...
          {formData.jobRolePreferences.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {formData.jobRolePreferences.map((role) => (
                <div
                  key={role}
                  className="flex items-center gap-1 px-2 py-1 rounded-full bg-cyan-950/50 border border-cyan-400/30 text-cyan-400 text-sm"
                >
                  {role}
                  <button
                    type="button"
          ...
          <div className="relative">
            <button
              ...
              className="flex w-full items-center justify-between rounded-lg border border-white/10 bg-white/5 p-3 text-left text-gray-300 hover:border-white/30"
            >
              <span>
                {formData.jobRolePreferences.length > 0
                  ? `${formData.jobRolePreferences.length} role(s) selected`
                  : "Select job roles"}
              </span>
              <ChevronDown
              ...
              <div className="absolute z-10 mt-1 w-full rounded-lg border border-white/10 bg-gray-900 shadow-lg">
                <div className="p-2">
                  <input
                    ...
                    className="w-full rounded-md border border-white/10 bg-white/5 p-2 text-sm text-white placeholder-gray-400 focus:border-cyan-400/50 focus:outline-none"
                  />
                </div>
                  ...
                  {filteredJobRoles.length > 0 ? (
                    filteredJobRoles.map((role) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          handleMultiSelect("jobRolePreferences", role)
                        }
      ...
      return (
        <div className="space-y-6">
          ...
          {formData.yearsOfExperience === "" && (
            <div className="rounded-lg border border-amber-400/20 bg-amber-950/20 p-3">
              <p className="text-sm text-amber-400">
                Please select your years of experience to continue
              </p>
            </div>
          )}
        </div>
