...
import { ArrowLeft, ArrowRight, BrainCircuit, Sparkles, User, Briefcase, Brain, Info, CloudCog } from "lucide-react";
...
import { mapOptionalToFormattedJSON, prepareArg, toUnixTimestamps, unixToDateString } from "../../../core/utils/canisterUtils";
...
export default function Register() {
    ...
    async function fetchUser() {
      ...
      if ("ok" in response) {
        ...
        setFormData({
          ...formData,
          name: user.full_name ?? "",
          dateOfBirth: user.date_of_birth ? unixToDateString(user.date_of_birth) : "",
  ...
  const nextStep = async () => {
    if (currentStep < totalSteps - 1) {
      ...
      setIsLoading(true);
      const response = await hirex_backend.register(prepareArg(formData.name), prepareArg(toUnixTimestamps(formData.dateOfBirth)), prepareArg(formData.yearsOfExperience), prepareArg(formData.educationLevel), prepareArg(formData.personalityTraits), prepareArg(formData.learningStyle), prepareArg(formData.jobRolePreferences), prepareArg(formData.jobSearchStatus), prepareArg(formData.roleLevel), prepareArg(formData.workMode), prepareArg(formData.companySize), prepareArg(formData.industriesOfInterest), prepareArg(formData.expectedLocation), [0]);
      setIsLoading(false);
      ...
      <div className="min-h-screen bg-black text-white">
        ...
        <main className="pt-16">
          <div className="flex min-h-[calc(100vh-4rem)]">
            {/* Left Column - Graphic */}
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="hidden w-1/2 bg-gradient-to-br from-gray-900 to-black p-6 lg:flex lg:items-center lg:justify-center">
            ...
            {/* Right Column - Form */}
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex w-full flex-col justify-center p-6 lg:w-1/2">
              <div className="mx-auto w-full max-w-xl rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-sm">
                ...
                <div className="mt-8 flex justify-between">
                  ...
                  {currentStep < totalSteps - 1 ? (
                    ...
                    <Button type="button" onClick={handleSubmit} className="bg-gradient-to-r from-cyan-400 to-violet-500 text-black hover:from-cyan-500 hover:to-violet-600">
                      Complete Profile
                      <ArrowRight className="ml-2 h-4 w-4" />
                    </Button>
                  )}
