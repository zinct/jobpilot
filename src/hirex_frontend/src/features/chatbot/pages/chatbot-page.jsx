...
export default function CareerChatbot() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content: "Hi John! I'm your AI career assistant. How can I help with your career journey today?",
    },
  ]);
  ...
  const handleSendMessage = async (e) => {
    try {
      ...
      if ("ok" in response) {
        const assistantMessage = { role: "assistant", content: response.ok };
  ...
  return (
    <div className="grid gap-6 md:grid-cols-3">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="md:col-span-2">
        <div className="flex h-[calc(100vh-12rem)] flex-col rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm">
          ...
          <div className="border-t border-white/10 p-4">
            <form onSubmit={handleSendMessage} className="relative">
              <input type="text" placeholder="Ask about your career..." value={inputValue} onChange={(e) => setInputValue(e.target.value)} disabled={isLoading} className={`w-full rounded-xl border ${isLoading ? "border-gray-700 bg-gray-800/30 text-gray-500" : "border-white/10 bg-white/5 text-white"} px-4 py-3 pr-12 placeholder-gray-400 focus:border-cyan-400 focus:outline-none focus:ring-1 focus:ring-cyan-400`} />
              <Button type="submit" size="icon" disabled={isLoading} className={`absolute right-2 top-1/2 -translate-y-1/2 ${isLoading ? "bg-transparent text-gray-600" : "bg-transparent hover:bg-white/10 text-cyan-400"}`}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="space-y-6">
        <div className="rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
          ...
          <div className="space-y-2">
            {["Interview preparation tips", "Salary negotiation strategies", "Career path planning", "Resume optimization", "Skill development advice"].map((topic, index) => (
              <button
                key={index}
