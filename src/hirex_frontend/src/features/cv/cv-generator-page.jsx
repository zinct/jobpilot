      ...
      <div className="grid gap-6 md:grid-cols-2">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm md:col-span-2">
          <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
            ...
            <Button
              ...
              onClick={() => {
                navigate("/dashboard/cv-generator/builder");
              }}
            >
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {["Professional", "Creative", "Technical", "Executive"].map((template, index) => (
              <div key={index} className="group cursor-pointer rounded-lg border border-white/10 bg-white/5 p-4 transition-colors hover:border-cyan-400/50">
                <div className="mb-3 aspect-[3/4] rounded-md bg-gradient-to-b from-gray-800 to-gray-900"></div>
                <h4 className="font-medium group-hover:text-cyan-400">{template}</h4>
